...
def part1(content):
    ...
    for idx in range(0, len(content)):
        for idy in range(0, len(content)):
            near_by = helper.get_neighbours(content, idx, idy, directions=glob.cardinal_directions, ignore_none=True)
...
def part2(content, point, memory={}):
    adjacent_dict = helper.get_neighbours(content, point[0], point[1], glob.cardinal_directions, True)
